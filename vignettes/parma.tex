...
Matlab) version of the cmaes solver of \citeasnoun{Hansen2006} is made
...
and analyzed among others by \citename{Dantzig1956} (\citeyear*{Dantzig1956},
\citeyear*{Dantzig1992}), \citeasnoun{Beale1955}, \citeasnoun{Dantzig1993},
\citeasnoun{Madansky1962} and \citeasnoun{Charnes1959}. An excellent
exposition of SP models in asset and liability management can be found in
\citeasnoun{Kouwenberg2006}. Generally speaking, stochastic programming
...
first proposed by \citeasnoun{Bernoulli1954} as a solution to the
St.Petersburg Paradox\footnote{Where the distinction between expected utility
and expected return was made.}, and formalized by \citeasnoun{VonNeumann1944}
...
approach. See for example \citeasnoun{Savage1962} for subjective expected
utility, \citeasnoun{Quiggin1982} and \citeasnoun{Schmeidler1989} for rank
dependent utility and \citeasnoun{Zadeh1965} for Fuzzy Logic.} to rational
decision making. Risk attitudes in expected utility theory are usually
measured by the Arrow-Pratt (see \citeasnoun{Arrow1963}) definitions of
...
the Safety-First criterion of \citeasnoun{Roy1952} which aimed at minimizing
...
in seminal papers by \citeasnoun{Artzner1999} and \citeasnoun{Acerbi2002a} on
risk and \citeasnoun{Rockafellar2006} on deviation, with the latter
...
\citeasnoun{Rockafellar2006} defined a set of axioms which functions in the
...
\citeasnoun{Artzner1999} provides the equivalent 'coherent' risk measure
...
\citeasnoun{Acerbi2002a} defined the family of spectral risk measures as
...
\citeasnoun{Rockafellar2006} defined a one-to-one relationship between
...
\citeasnoun{Markowitz1952} ushered in the era of modern portfolio management
...
by \citeasnoun{Hanoch1969}\footnote{The criticism was in fact also aimed at
...
\citeasnoun{James1956} provide for a shrinkage estimator, \citeasnoun{Black1992} a
semi-Bayesian approach while \citeasnoun{Michaud1989} a general criticism of the
...
of this, \citeasnoun{Konno1991} introduced a piece-wise linear formulation of
the absolute deviation function as an alternative to the
\citeasnoun{Markowitz1952} method. The standard NLP objective function may be
formulated as:
...
\end{gathered}
\end{equation}
which \citeasnoun{Konno1991} reduced to the following piece-wise linear problem:
...
n$ diagonal matrices stacked together\footnote{\citeasnoun{Feinstein1993}
...
\citeasnoun{Konno1993}, and semi-absolute deviation first suggested by
\citeasnoun{Speranza1993} who showed that the mean semi-deviation is a half
...
The MiniMax model of \citeasnoun{Young1998a}, aims to minimize the maximum loss,
...
\citeasnoun{Young1998a} only considered the problem in light of historical
...
\citeasnoun{Markowitz1952} in a reference to semi-standard deviation. This
was later formalized into a very general class of measures by
\citeasnoun{Stone1973}, and the Lower Partial Moment (\emph{LPM}) framework
of \citeasnoun{Fishburn1977} which, in the continuous case, may be defined
as:
\begin{equation}
...
the power of $\frac{1}{a}$. \citeasnoun{Fishburn1977} derived a utility
...
\end{array}
\end{equation}
where $k$ is a positive constant. \citeasnoun{Harlow1989} describe an asset
...
theory, \citeasnoun{Bawa1977}, \citeasnoun{Bawa1978} and
\citeasnoun{Fishburn1977} showed that stochastic dominance is equivalent to
all degrees
of n-degree LPM.\\
...
Safety-First  model of \citeasnoun{Roy1952}, $a=1$ the below target shortfall
...
some problems in the optimization strategy. \citeasnoun{Nawrocki1989} devised
...
benchmark rate\footnote{However, \citename{Brogan2005}
...
\citeasnoun{Holthausen1981} extended the LPM model to include a non-linear
...
Since the report by the Group of Thirty \citeasnoun{G301993}, the use of
...
according to \citeasnoun{Rockafellar2000}.} which is a coherent and convex
risk measure belonging to the class of spectral risk measures of
\citeasnoun{Acerbi2002}. Formally, a spectral risk measure $M_\psi$ is a
...
losses greater than VaR). \citeasnoun{Cotter2006} investigated alternative
...
weighting schemes were superior to the standard CVaR, \citeasnoun{Dowd2008}
...
\citeasnoun{Rockafellar2006} considered the so called mixed-CVaR problem
...
(due to \citeasnoun{Rockafellar2000}):
\begin{equation}
\begin{gathered}
...
\citeasnoun{Biglova2004} proposing the Rachev Ratio as the upper to lower
CVaR for which they provide a mixed integer representation in
\citeasnoun{Stoyanov2007} (modified by \citeasnoun{Konno2011} for cases when
...
motion with zero drift discussed in \citeasnoun{Douady1999}, there is no
...
\citeasnoun{Magdon-Ismail2004}, the measure considered here and its
optimization is based on \citeasnoun{Chekhlov2005}. The problem may be posed
as the following LP:
\begin{equation}
\begin{gathered}
...
the \emph{parmaspec} options }, something also discussed in \citeasnoun{Brogan2005}.
...
\citeasnoun{Markowitz1952} may be represented as:
...
section. The interested reader should consult for example \citeasnoun{Stoyanov2007}
...
\citeasnoun{Charnes1962}, while for nonlinear cases the main contributions
can be traced to \citeasnoun{Dinkelbach1967} and
\citename{Schaible1976a}(\citeyear*{Schaible1976a},
\citeyear*{Schaible1976b}). More recently, \citeasnoun{Stoyanov2007} provided
...
in \citeasnoun{Jacobs2006} which double the size of the problem and require
